...
lambdaf = lambda*exp(alpha*p);

LUf = LU(h,p,dx,dy,lambdaf);
...
lambdac = lambda*exp(alpha*pc);
p_rhsc = p_rhsc + LU(hc,pc,dxc,dyc,lambdac);
...
hc(ii,jj) = (h(ii,jj).^3) ./lambda(ii,jj) ;

hr(ii,jj) = 0.5*( (h(3:nx,jj).^3)  ./lambda(3:nx,jj) + hc(ii,jj))/dx.^2;
hl(ii,jj) = 0.5*( (h(1:nx-2,jj).^3)./lambda(1:nx-2,jj) + hc(ii,jj))/dx.^2;

hu(ii,jj) = 0.5*( (h(ii,jj + 1).^3) ./lambda(ii,jj + 1) + hc(ii,jj))/dy.^2;
hd(ii,jj) = 0.5*( (h(ii,jj - 1).^3) ./lambda(ii,jj - 1) + hc(ii,jj))/dy.^2;
